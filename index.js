// Your code here
function createEmployeeRecord([emptyFirstName, emptyFamilyName, emptyTitle, emptyPayRate]) {
    let employeeRecord = {
        firstName: emptyFirstName,
        familyName: emptyFamilyName,
        title: emptyTitle,
        payPerHour: emptyPayRate,
        timeInEvents: [],
        timeOutEvents: []
    }
...
function hoursWorkedOnDate(employeeObject, date){
    let startTime
    let stopTime
    for (let i = 0;i<employeeObject.timeOutEvents.length; i++){
        if (employeeObject.timeOutEvents[i].date ===date){
            startTime = employeeObject.timeInEvents[i].hour.toString();
            stopTime = employeeObject.timeOutEvents[i].hour.toString();
        }
    }
